...
```r
?arrange
arrange(gapminder, year ) # default: increasing
```
...
```r
arrange(gapminder, desc(year)) # descending order
```
...
If output of piping should not be automatically be put in the first place for the subsequent function:

dat %>% function(a = 2, b=.) # .dot puts input here
